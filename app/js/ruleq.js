...
//	console.log('nguhanha[nguhanhc[thienc]]: '+nguhanha[nguhanhc[thienc]]);
	...
	var menhtuod,fontcolorxe;
		if(cung['cung']=='Chấn'){menhtuod=', thuộc dương Mộc';fontcolorxe = '#58D68D';}
		if(cung['cung']=='Tốn'){menhtuod=', thuộc âm Mộc';fontcolorxe = '#58D68D';}
		if(cung['cung']=='Ly'){menhtuod=', thuộc âm Hoả';fontcolorxe ='#EC7063';}
		if(cung['cung']=='Cấn'){menhtuod=', thuộc dương Thổ';fontcolorxe = '#F7DC6F';}
		if(cung['cung']=='Khôn'){menhtuod=', thuộc âm Thổ';fontcolorxe = '#F7DC6F';}
		if(cung['cung']=='Càn'){menhtuod=', thuộc dương Kim';fontcolorxe ='#B2BABB';}
		if(cung['cung']=='Đoài'){menhtuod=', thuộc âm Kim';fontcolorxe ='#B2BABB';}
		if(cung['cung']=='Khảm'){menhtuod=', thuộc dương Thuỷ';fontcolorxe ='#AED6F1';}
	

	if(dlal.substr(-2)=="ng"){
	retcurn ='Bạn xem ngày '+nday+' tháng '+nmonth+', năm '+nyear+' dương lịch</br>Cung phi <span style="color:'+fontcolorxe+'">'+(cung['cung'])+' '+menhtuod+'</span> '+(cung['huong'])+', ngũ hành '+menhtuoi+', bát tự của bạn sẽ rơi vào giờ '+counttime.toLowerCase()+', ngày '+dayName.toLowerCase()+' (tức ngày '+day+'), tháng '+(thang.toLowerCase())+' (tức tháng '+month_name.toLowerCase()+') , năm '+(thiencan.toLowerCase())+' '+yearten+' '+icceeer+'.<br>Thuộc '+iscanduongornam+' nạp âm của bạn là can <b>'+chic+'</b>, chi <b>'+thienc+'</b>, là '+cketcanchi+', tháng sinh của bạn là tháng đại diện cho tính chất '+ynghiathang[month]+', '+ynghiathangex[month]+'';
	}else{
	retcurn ='Bạn xem ngày '+day+', tháng '+month_name+', năm '+year+' âm lịch, có cung phi <span style="color:'+fontcolorxe+'">'+(cung['cung'])+' '+menhtuod+'</span> '+(cung['huong'])+', ngũ hành '+menhtuoi+', bát tự của bạn là giờ '+(counttime.toLowerCase())+', ngày '+(dayName.toLowerCase())+', tháng '+(thang.toLowerCase())+' - năm '+(thiencan.toLowerCase())+' '+(yearten.toLowerCase())+'. '+nhamngay+'<br>Thuộc '+iscanduongornam+' chi: <b>'+thienc+'</b> nạp âm của bạn là can: <b>'+chic+'</b>, là '+cketcanchi+', tháng sinh của bạn là tháng đại diện cho tính chất '+ynghiathang[month]+', '+ynghiathangex[month]+'';
	}
...
function regdomr(json={},tcc,dcc,dcx){
document.querySelector('#tv').innerHTML='Đang xử lý kết quả thu thập được...';
$.ajax({
...
var rtname= (urlParams.get('sex')=='Nam'?'rdNam':'rdNu')+'_'+urlParams.get('ngay')+'_'+urlParams.get('thang')+'_'+tcc+'_'+dcc+'_'+dcx;
	//  //api.github.com/repos/thienco/thienco.github.io/contents
fetch('https://raw.githubusercontent.com/thienco/thienco.github.io/main/database/'+rtname+'', {
  method: 'get'}).then(res=>res.json(),function(data){regdomr({sha:null},tcc,dcc,dcx);})
  .then(json =>{
	try{if(json.html){paswrreponse(json.html);}} catch(e){regdomr({sha:null},tcc,dcc,dcx);}
	
if( json.sha){

paswrreponse(json.html);

}
	
},function(data){
regdomr({sha:null},tcc,dcc,dcx);
